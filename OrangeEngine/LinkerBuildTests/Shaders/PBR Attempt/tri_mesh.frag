...
void main() {
    ...
    float roughness = 0.1f;//texture(texSampler[2], fragTexCoord).r;
    float ao        = texture(texSampler[3], fragTexCoord).r;
    float metallic  = 0;//texture(texSampler[4], fragTexCoord).r;


    // For BC5 it used (rg) [-1, 1] range  (X^2 + Y^2 + Z^2) == 1
    vec3 N;
	N.xy = (texture(texSampler[1], fragTexCoord).gr * 2.0) - 1.0;
...
float ss = pow( max(dot(V, reflect(N,L)), 0.0), 8);
float dd = max(dot(N, L), 0.0);
    outColor.rgb = dd.rrr + ss.rrr;
}